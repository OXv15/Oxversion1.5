...
body('class="ox-bg-dark"');
    ...
    div('class="login-form p-5"');
    	form('method="POST" action="register.php"');
    		input('class="form-control text-center p-3 m-1" type="text" name="firstname" placeholder="Firstname" required');
    		input('class="form-control text-center p-3 m-1" type="text" name="lastname" placeholder="Lastname" required');
    		input('class="form-control text-center p-3 m-1" type="text" name="username" placeholder="Username" required');
    		input('class="form-control text-center p-3 m-1" type="email" name="email" placeholder="Email" required');
    		input('class="form-control text-center p-3 m-1" type="number" name="phone" placeholder="Phone" required');
    		if (filter_has_var(INPUT_POST, 'register')) {
				if ($_POST['password'] == $_POST['cpassword']) {
					echo "<script>window.location= 'register.php'</script>";
				}else{
					input('class="form-control border border-danger ox-bg-dark text-center text-white p-3 m-1" type="password" name="password" placeholder="Password dosent match. retry!" required');
    				input('class="form-control border border-danger ox-bg-dark text-center p-3 m-1" type="password" name="cpassword" placeholder="Confirm password" required');
				}
			}else{
				input('class="form-control text-center p-3 m-1" type="password" name="password" placeholder="Password" required');
				input('class="form-control text-center p-3 m-1" type="password" name="cpassword" placeholder="Confirm password" required');
			}
    		input('class="form-control dark btn text-white text-center p-4 m-1" type="submit" name="register" value="Register"');
    		p('class="text-center text-white"');echo "Already have an account?";a('class="text-center text-info"href="index-login.php"'); echo " Login now"; _a();_p();
    	_form();
    _div();
